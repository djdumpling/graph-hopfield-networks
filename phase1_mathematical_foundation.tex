...
Under our undirected graph convention (each edge counted exactly once in $E$), we have:
\begin{align}
...
\end{align}
where line 3 uses: (i) each node $v$ appears in exactly $d_v$ edges, contributing $d_v x_v^T x_v$, and (ii) since $A$ is symmetric, $\tr(X^T A X) = \sum_{u,v} A_{uv} x_u^T x_v = 2\sum_{(u,v) \in E} x_u^T x_v$.
...
\begin{itemize}
    \item Linear function: $g(x) = M x$ (affine)
...
\begin{proof}
The graph Laplacian $L$ is positive semi-definite (all eigenvalues $\geq 0$). 
By PSD, for any $X, Y$:
\[
...
Then the (block) Hessian of the node-wise Hopfield retrieval map satisfies
\[
0 \preceq \Sigma(p_v(X)) \preceq I
\quad\Longrightarrow\quad
0 \preceq M^\top \Sigma(p_v(X)) M \preceq \|M\|^2 I.
\]
Consequently the (block) Hessian of $E_{\text{GH}}$ admits the uniform lower bound
\[
\nabla^2 E_{\text{GH}}(X) \;\succeq\; \bigl(1-\beta\|M\|^2\bigr)\,I \;+\; 2\lambda\,(L\otimes I_d),
\]
hence:
\begin{itemize}
\item if $\beta\|M\|^2 \le 1$, then $E_{\text{GH}}$ is convex;
\item if $\beta\|M\|^2 < 1$, then $E_{\text{GH}}$ is $\mu$-strongly convex with $\mu := 1-\beta\|M\|^2$
(and in particular has a unique global minimizer).
\end{itemize}

\textbf{Note on the Laplacian term:} The Laplacian $L$ is positive semi-definite but has a nontrivial nullspace (constant signals across all nodes). However, the global strong convexity of $E_{\text{GH}}$ comes from the $+I$ term (the $\frac{1}{2}\|X\|_F^2$ quadratic regularizer), which dominates the negative curvature from the Hopfield term when $\beta\|M\|^2 < 1$. The Laplacian term $2\lambda(L \otimes I_d)$ adds additional positive curvature in directions orthogonal to the nullspace.

\item \textbf{(Difference-of-convex decomposition always exists)}  
Because $\nabla E_{\text{GH}}$ is $L_{\text{lip}}$-Lipschitz, the function
\[
H(X) := \frac{L_{\text{lip}}}{2}\|X\|_F^2 - E_{\text{GH}}(X)
\]
is convex (standard fact: $L$-smooth $\iff$ $X\mapsto \tfrac{L}{2}\|X\|^2 - f(X)$ convex).
Therefore,
\[
...
If $\beta\|M\|^2<1$, then $E_{\text{GH}}$ is strongly convex and gradient descent converges globally and linearly.
Without this condition, the objective can be nonconvex; gradient descent still converges to stationary points
(Theorem 5.7), and DC algorithms such as CCCP/MM can be used to obtain monotone descent to critical points
...
\[
L_{\mathrm{lip}}\;=\;\beta\|M\|^2\;+\;1\;+\;2\lambda\|L\|,
\]
i.e.
\[
...
and satisfies the Loewner bound $\Sigma(p)\preceq \diag(p)\preceq I$.
...
this implies $s$ is $\beta\|M\|^2$-Lipschitz:
\[
\|s(x)-s(y)\|\le \beta\|M\|^2\|x-y\|\qquad\forall x,y\in\mathbb{R}^d.
\]
...
\le
\sum_{v=1}^n (\beta\|M\|^2)^2 \|x_v-y_v\|^2
=
(\beta\|M\|^2)^2\|X-Y\|_F^2,
\]
so
\[
\|S(X)-S(Y)\|_F\le \beta\|M\|^2\|X-Y\|_F.
\]
Plugging this into \eqref{eq:lip_reduce_to_S} gives
\[
\|\nabla E_{\mathrm{GH}}(X)-\nabla E_{\mathrm{GH}}(Y)\|_F
\le
\bigl(\beta\|M\|^2+1+2\lambda\|L\|\bigr)\|X-Y\|_F,
\]
as claimed.
\end{proof}
...
Assume $E_{\mathrm{GH}}$ is bounded below on $\mathbb{R}^{n\times d}$, and let $L_{\mathrm{lip}}$ be as in
...
For the \textbf{normalized Laplacian} $L = I - D^{-1/2} A D^{-1/2}$, we have $\|L\| \le 2$:
\begin{equation}
\eta < \frac{2}{\beta \|M\|^2 + 1 + 4\lambda}
\label{eq:step_size_bound_normalized}
\end{equation}

For the \textbf{unnormalized Laplacian} $L = D - A$ with maximum degree $d_{\max}$, we have $\|L\| \le 2d_{\max}$:
\begin{equation}
\eta < \frac{2}{\beta \|M\|^2 + 1 + 4\lambda d_{\max}}
...
\item \textbf{Fixed-point iteration:} $X^{(t+1)} = T(X^{(t)})$ (defined below) does \emph{not automatically} imply energy descent. Convergence is guaranteed \emph{only when $T$ is a contraction}, i.e., under the sufficient condition $\beta\|M\|^2 + 2\lambda\|L\| < 1$ (Theorem~\ref{thm:fp_contraction}).
\end{enumerate}
...
\[
\mathrm{Lip}(T)\;\le\;\beta\|M\|^2\;+\;2\lambda\|L\|.
\]
In particular, if the sufficient condition
\[
\beta\|M\|^2 + 2\lambda\|L\| \;<\; 1
\]
...
\qquad
\rho := \beta\|M\|^2 + 2\lambda\|L\| \in (0,1).
\]
\end{theorem}

\begin{proof}
Decompose $T(X)=S(X)-2\lambda L X$ where, as before, $[S(X)]_v=s(x_v)$ and $s(x)=M^\top\softmax(\beta Mx)$.
From the Jacobian calculation in Proposition~\ref{prop:lipschitz}, $s$ is $\beta\|M\|^2$-Lipschitz:
\[
\|s(x)-s(y)\|\le \beta\|M\|^2\|x-y\|\quad\forall x,y.
\]
Therefore, row-wise,
\[
\|S(X)-S(Y)\|_F \le \beta\|M\|^2\|X-Y\|_F.
...
\le
\bigl(\beta\|M\|^2 + 2\lambda\|L\|\bigr)\|X-Y\|_F.
\]
Hence $\mathrm{Lip}(T)\le \beta\|M\|^2+2\lambda\|L\|$. If this quantity is $<1$,
Banach's fixed-point theorem implies existence and uniqueness of $X^\star$ and geometric convergence with rate $\rho$.
\end{proof}

\begin{corollary}[Convenient sufficient condition via normalized Laplacian]
If $L$ is the normalized Laplacian, then $\|L\|\le 2$, so a sufficient condition for contraction is
\[
\beta\|M\|^2 + 4\lambda < 1.
\]
If $L$ is the unnormalized Laplacian, then $\|L\|\le 2d_{\max}$, so a sufficient condition is
\[
\beta\|M\|^2 + 4\lambda d_{\max} < 1.
\]
\end{corollary}
...
\begin{proof}
From the descent inequality proved in Theorem 5.7,
\[
...
Assume $E_{\text{GH}}$ is $\mu$-strongly convex and $L_{\text{lip}}$-smooth (e.g., under $\mu=1-\beta\|M\|^2>0$
...
\begin{proof}
This is the standard contraction result for gradient descent on $\mu$-strongly convex, $L$-smooth functions.
One proof route: combine strong convexity with smoothness to obtain
$\langle \nabla E(X)-\nabla E(Y), X-Y\rangle \ge \mu\|X-Y\|_F^2$ and
$\|\nabla E(X)-\nabla E(Y)\|_F \le L_{\text{lip}}\|X-Y\|_F$,
then show the GD update map is a contraction with factor $(1-\eta\mu)$ for $\eta\le 1/L_{\text{lip}}$.
\end{proof}
...
\]
Since $\max_i m_i^\top x_v \le \|M\|\,\|x_v\|$ (where $\|M\|$ is the spectral norm), we have
\[
...
\end{itemize}
If $E_{\text{GH}}$ is convex (e.g. $\beta\|M\|^2\le 1$), then \emph{every} critical point is a global minimizer;
...
If $E_{\text{GH}}$ is strongly convex (e.g. $\beta\|M\|^2<1$), then gradient descent converges to the unique
global minimizer $X^\star$ from any initialization.
If the fixed-point map $T$ is a global contraction (Theorem~2), then fixed-point iteration converges to the unique
fixed point from any initialization.
\end{prop}
