...
\]
The covariance matrix $\Sigma(p)$ is positive semidefinite (it is the covariance matrix of a categorical distribution) and satisfies the tighter bound $\Sigma(p) \preceq \frac{1}{2}I$ (since $\max_i p_i(1-p_i) \le \frac{1}{4}$ for any probability vector $p$). Therefore,
\[
...
and satisfies the tighter bound $\Sigma(p)\preceq \tfrac{1}{2}I$ (since $\max_i p_i(1-p_i) \le \tfrac{1}{4}$ for any probability vector $p$).
...
\]
Using the tighter bound $\|\Sigma(p)\|\le 1/2$ would give $\tfrac{\beta}{2}\|M\|^2 + 4\lambda d_{\max} < 1$, but the fixed-point operator analysis uses the looser bound for consistency with the gradient Lipschitz analysis.
\end{corollary}
